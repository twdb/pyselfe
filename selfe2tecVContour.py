...
xy = loadtxt(profilefile,skiprows=10,usecols=(0,1),unpack=True)
nxy = xy.shape[1]
xy=xy.transpose()
...
for dt in range(t.size):
    ...
    for j in range(model.nlevels):
        for i in range(nxy):
            fid.write(X[i,j].__str__() + ' ' + Z0[i,j].__str__() + ' ' + (data[dt,i,j,0]).__str__() + '\n')
fid.close()
